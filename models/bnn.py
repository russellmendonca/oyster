...
class BNN:
    ...
    with ensembling).
    """
    def __init__(self,  params , context_dim, obs_dim, act_dim, ada_state_dynamics_pred, ada_rew_pred,
                 fast_adapt_steps , fast_adapt_lr ,reg_weight , fixed_preupdate_context,
                 meta_batch_size =10, clip_val_inner_grad= 10, clip_val_outer_grad = 10):
        ...
        self.fixed_preupdate_context = fixed_preupdate_context
        
        self.ada_state_dynamics_pred = ada_state_dynamics_pred
        self.ada_rew_pred = ada_rew_pred
        assert self.ada_state_dynamics_pred or self.ada_rew_pred

        # meta training hyper-params
        self.meta_batch_size = meta_batch_size
        self.fast_adapt_steps = fast_adapt_steps
        self.fast_adapt_lr = fast_adapt_lr
        self.clip_val_inner_grad = clip_val_inner_grad
        self.clip_val_outer_grad = clip_val_outer_grad
        self.reg_weight = reg_weight
        self.pred_dynamics = pred_dynamics
    ...
    # def train_loss_vector(self, input_data, output_data):
    #
    #     '''
    #     :param input_data:  placeholder for [obs, action, context] data
    #     :param output_data: placeholder for [reward, delta_obs] data
    #     :param context:     context variable
    #     :return:
    #     '''
    #     train_loss = self._compile_losses(input_data, output_data, inc_var_loss=True)
    #     train_loss += tf.add_n(self.decays)
    #     train_loss += 0.01 * tf.reduce_sum(self.max_logvar) - 0.01 * tf.reduce_sum(self.min_logvar)
    #
    #     #train_loss = tf.reduce_sum(self._compile_losses(input_data, output_data, inc_var_loss=False))
