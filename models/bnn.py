...
class BNN:
    ...
    def finalize(self, optimizer, optimizer_args=None, *args, **kwargs):
        ...
        with self.sess.as_default():
            with tf.variable_scope(self.name):
                ...
                self.max_rew_logvar = tf.Variable(np.ones([1, 1])*(0.5 + np.log(1e-10 + self.reward_prediction_weight**2)),
                                              dtype=tf.float32,
                                              name="max_rew_log_var")
                self.min_rew_logvar = tf.Variable(np.ones([1, 1]) *(-10 + np.log(1e-10 + self.reward_prediction_weight**2)),
                                              dtype=tf.float32,
                                              name="min_rew_log_var")
                

                self.context = tf.Variable(np.ones(self.context_dim) , dtype = tf.float32 , name = 'context')
        ...
        with tf.variable_scope(self.name):
            ...
            self.train_input = tf.placeholder(dtype=tf.float32,
                                              shape=[self.num_nets, None, self.layers[0].get_input_dim() - self.context_dim],
                                              name="train_input")
            ...
            self.post_adapt_val_dict = self._get_model_loss_dict(val_input, updated_contexts, val_target,
                                                            add_rew_pred=self.ada_rew_pred, add_state_pred= self.ada_state_dynamics_pred)
            self.post_adapt_val_loss = self.post_adapt_val_dict['total_model_loss']
           
            self.total_loss = self.post_adapt_val_loss + (self.reg_weight/self.meta_batch_size)*tf.norm(self.updated_contexts - self.context)+\
            ...
            self.gvs = [(tf.clip_by_value(grad, -self.clip_val_outer_grad, self.clip_val_outer_grad), var) for grad, var in gvs]
            ...
            with tf.variable_scope(self.name + '_regular'):
                ...
                self.regular_train_loss =  train_loss+ 0.01 * tf.reduce_sum(self.max_logvar) - 0.01 * tf.reduce_sum(self.min_logvar)
    ...
    def get_updated_contexts(self, train_input, train_target):
    ...
    def save_model(self, _dir):
        ...
        pickle.dump(val_dict, open(_dir + 'meta_model.pkl', 'wb'))
    ...
    def _compile_losses(self, inputs, contexts, targets):
        ...
        rew_mean, rew_logvar, state_mean, state_logvar = self.get_weighted_mean_logvar(self._compile_output_layer(inputs))
