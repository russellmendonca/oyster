...
from smrl.helper import append_context_to_inputArray
...
class FakeEnv:
	...
	def step(self, obs, act, context, deterministic=False):
		...
		inputs_with_context = append_context_to_inputArray(inputs, context) if (context is not None) else inputs
		ensemble_model_means, ensemble_model_vars = self.model.predict(inputs_with_context, factored=True)
		...
		model_inds = [0]  # NO ENSEMBLE FOR NOW!
